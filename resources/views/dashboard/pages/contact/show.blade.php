...
<div class="email-app mb-4">
    <nav>
      ...
      <ul class="nav">
        ...
        <li class="nav-item">
          <a class="nav-link" href="{{url('messages')}}"><i class="fa fa-rocket"></i> Envoyer</a>
        </li>
        <li class="nav-item">
    ...
    <main class="message">
      ...
      <div class="details">
        <div class="title">{{$contact->Sujet}}.</div>
        <div class="header">
          <img class="avatar" src="{{asset($contact->Image)}}">
          <div class="from">
            <span>{{$contact->Nom}} {{$contact->Prenom}}</span>
            {{$contact->Email}}
          </div>
          <div class="date">{{$contact->created_at->format('d-M-Y')}}, <b>{{$contact->created_at->format('H:i')}}h</b></div>
        </div>
        <div class="content">
          <p>
            {{$contact->Message}}
        ...
        <form method="post" action="{{url('replyMessage', $contact->id)}} ">
        {{ csrf_field() }}
          <div class="form-group">
            <textarea class="form-control" id="message" name="message" rows="6" placeholder="Click here to reply"></textarea>
          </div>
          <div class="form-group">
            <button tabindex="3" type="submit" class="btn btn-success">Envoyer message</button>
          </div>
        </form>
      </div>
    </main>
